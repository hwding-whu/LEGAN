...
class Discriminator(nn.Module):
    def __init__(self, M=28):
        ...
        self.linear = nn.Linear(M // 4 * M // 4 * 256, 1)
        self.initialize()
...
class Generator28(Generator):
    def __init__(self, z_dim):
        super().__init__(z_dim, M=3)


class Discriminator28(Discriminator):
    def __init__(self):
        super().__init__(M=28)

class OptimizedResGenBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.residual = nn.Sequential(
            ...
            nn.Conv2d(in_channels, out_channels, 2, stride=1, padding=1),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(),
            nn.Conv2d(out_channels, out_channels, 2, stride=1, padding=1),
        )
...
class ResGenerator28(nn.Module):
    def __init__(self, z_dim):
        super().__init__()
        self.z_dim = z_dim
        self.linear = nn.Linear(z_dim, 3 * 3 * 128)

        self.blocks = nn.Sequential(
            ResGenBlock(128, 128),
            OptimizedResGenBlock(128, 128),
            ResGenBlock(128, 128),
        )
        self.output = nn.Sequential(
            ...
            # nn.BatchNorm2d(256),
            # nn.ReLU(True),
            # nn.Conv2d(256, 3, 3, stride=1, padding=0),
            nn.Tanh(),
        )
        self.initialize()
...
class ResDiscriminator28(nn.Module):
    def __init__(self):
        super().__init__()
